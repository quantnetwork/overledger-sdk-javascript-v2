...
class CognitoUserImpl {
    ...
    constructor(username: string, pool: AmazonCognitoIdentity.CognitoUserPool, secret_key: string) {
        ...
        return await authenticationHelper.getLargeAValue((errOnAValue, aValue) => {
            ...
            this.client.makeUnauthenticatedRequest('initiateAuth', jsonReq, (err, initiateAuthResult) => {
                ...
                authenticationHelper.getPasswordAuthenticationKey(this.username, authDetails.getPassword(), serverBValue, salt, (errOnHkdf, hkdf) => {
                    ...
                    const signatureString = util.crypto.hmac(hkdf, util.buffer.concat([new util.Buffer(this.pool.getUserPoolId().split('_')[1], 'utf8'),
    ...
    private calculateSecretHash(client_id: string, secret_key: string, user_name: string): string {
        const crypto = require('crypto');
        const result = crypto.createHmac('SHA256', secret_key)
