...
describe('Unit Tests:', () => {
    ...
    test('Can construct the DLT', () => {
        ...
        sdk.dlts[DltNameOptions.SUBSTRACT].setAccount({
            privateKey: mockSeed,
        },);

        expect(sdk.dlts[DltNameOptions.SUBSTRACT].name).toEqual(DltNameOptions.SUBSTRACT);
        expect(sdk.dlts[DltNameOptions.SUBSTRACT].account.privateKey).toEqual(mockSeed);
    });
    ...
    test('Can sign a transaction', async () => {
        ...
        const substractPreparedTransaction = require('./resources/substract-prepared-transaction.json');
        let signedTransaction = (await sdk.sign(DltNameOptions.SUBSTRACT, substractPreparedTransaction)).signedTransaction;
